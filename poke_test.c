...
int main(){
    ...
    if(check==1){ //쓰기 모드로 시작
        ...
        int p_num=pokemon_choice(0)-1; //포켓몬 정하기 함술
        fprintf(fp,"%d %d %d \n",&monster_ball,&good_spray,&poke_num);
        fprintf(fp,"%s %s %d %d %d",id[p_num].name,id[p_num].special_name,id[p_num].color,id[p_num].ad,id[p_num].hp);
        go_adventure(0);
    }
    else{ //이어서 시작
        fp = fopen("pokemon_directory.txt","r");
        fscanf(fp,"%d %d %d",&monster_ball,&good_spray,&poke_num); //아이템 부르기
